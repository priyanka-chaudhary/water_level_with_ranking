...
if __name__ == '__main__':
    ...
    path_loc = "/cluster/work/igp_psr/pchaudha/flood/rank/exp/three_pair_loader/three_pair_loader_2/runs/fold4/"
    ...
    # check file is json
    # Regression task
    with open('/cluster/work/igp_psr/pchaudha/flood/rank/k/k4/gt_train_reg.json') as f:
        train = json.load(f)
    ...
    with open('/cluster/work/igp_psr/pchaudha/flood/rank/k/k4/gt_train_rank.json') as f:
        train = json.load(f)
    strJson = json.dumps(train)
    ...
    # check file is json
    with open('/cluster/work/igp_psr/pchaudha/flood/rank/k/k4/gt_val_reg.json') as v1:
        val = json.load(v1)
    ...
    # check file is json
    with open('/cluster/work/igp_psr/pchaudha/flood/rank/k/k4/gt_val_rank.json') as v2:
        val2 = json.load(v2)
    ...
    train3 = PairDataset(img_path=train_img_path_2, labels=train_labels_2, transforms=data_transform)
    validation3 = PairDataset(img_path=val_img_path_2, labels=val_labels_2, transforms=data_transform)

    train_one = torch.utils.data.DataLoader(train1, **params)
    train_pair = torch.utils.data.DataLoader(train2, **params)
    val_one = torch.utils.data.DataLoader(validation1, **params)
    val_pair = torch.utils.data.DataLoader(validation2, **params)

    #new added for loading more than 1 rank dataloader
    train_pair2 = torch.utils.data.DataLoader(train3, **params)
    val_pair2 = torch.utils.data.DataLoader(validation3, **params)

    # new added for loading 3rd rank dataloader
    train_pair3 = torch.utils.data.DataLoader(train3, **params)
    val_pair3 = torch.utils.data.DataLoader(validation3, **params)

    t_one = itertools.chain(train_one,train_one,train_one,train_one)
    v_one = itertools.chain(val_one,val_one,val_one,val_one)


    dataloaders = {
        'train': [train_one, train_pair, train_pair2, train_pair3],
        'validation': [val_one, val_pair, val_pair2, val_pair3]
    }

    #TODO
    #Calculate the length of datasets here
    t_reg_len = 4*len(train1)
    v_reg_len = 4*len(validation1)
    data_length = {
    ...
    def train_model(model, loss_func, optimizer, scheduler, num_epochs=200):
        ...
        for epoch in range(num_epochs):
            ...
            for phase in ['train', 'validation']:
                ...
                for i, data in enumerate(zip(itertools.cycle(dataloaders[phase][0]), dataloaders[phase][1], dataloaders[phase][2], dataloaders[phase][3])):
                #for i, data in enumerate(itertools.zip_longest(myCycle(dataloaders[phase][0]), dataloaders[phase][1])):
                    ...
                    x_3 = dataset_3[0]
                    y_3 = dataset_3[1]
                    y_3_list = y_3.squeeze().tolist()

                    # 3rd pair dataset
                    dataset_4 = data[3]
                    x_4 = dataset_4[0]
                    y_4 = dataset_4[1]
                    y_4_list = y_4.squeeze().tolist()
                    ...
                    #new pair dataset
                    input3 = x_3.to(device)
                    target3 = torch.FloatTensor(gt_list2)
                    target3 = target3.to(device)

                    # 3rd pair dataset
                    input4 = x_4.to(device)
                    target4 = torch.FloatTensor(gt_list3)
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        x3, x4 = model(input3)
                        loss3 = criterion(x3, x4, target3)

                        x5, x6 = model(input4)
                        loss4 = criterion(x5, x6, target4)


                        # backward + optimize only if in training phase
                        if phase == 'train':
                            loss = loss1 + (flag.LAMBDA * (loss2 + loss3 + loss4))
                            loss.backward()
                            optimizer.step()
                    ...
                    running_loss3 += loss3.item() * x3.size(0)
                    running_loss4 += loss4.item() * x4.size(0)
                ...
                epoch_loss = (running_loss1 / data_length[phase][1]) + flag.LAMBDA*((running_loss2 / n_samples) + (running_loss3 / n_samples) +
                                                                                    (running_loss4 / n_samples))
                reg_loss = (running_loss1 / data_length[phase][1])
                rank_loss = flag.LAMBDA*((running_loss2 / n_samples) + (running_loss3 / n_samples) +
                                         (running_loss4 / n_samples))
                ...
                if phase == 'validation' and epoch_loss < best_loss:
                    ...
                    best_model_wts = copy.deepcopy(model.state_dict())
                    model_p = path_loc + "/rank_model_fold4.pth"
